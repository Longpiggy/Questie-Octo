...
function MM:RefreshVisualSettings()
  for _,pin in pairs(self.frames or {}) do
    if pin.itemStartArea then
      if QuestieOcto.Visuals then QuestieOcto.Visuals:ClearPin(pin,pin.lastAlpha or 1) end
    elseif pin.entries and next(pin.entries) then
      ...
      if QuestieOcto.Visuals then
        QuestieOcto.Visuals:SetAlpha(pin,pin.lastAlpha or 1)
      end
    end
  end
...
function MM:RescaleIcons(changedKey,changedValue)
  ...
  -- Presentation-only rescale. Keeping the existing texture/state intact is
  -- essential for Full Nodes: changing the global slider must never swap back
  -- to normal Questie objective artwork.
  for _,pin in pairs(self.frames or {}) do
    ResizePin(pin)
  end
...
function MM:ApplySettings()
  local settings=Settings()
  -- Keep the renderer alive for independently controlled service markers and rares.
  -- enableMiniMapIcons controls quest markers only.
  self.enabled=true
  self.globalScale=tonumber(settings:Get("globalMiniMapScale")) or 1
...
function MM:OnSettingChanged(key,value)
  if key=="enableMiniMapIcons" then
    self.enabled=true
    self:RefreshPlan()
    return
  end

  if key=="enableObjectives" or key=="enableTurnins" or key=="enableAvailable" or key=="showPvPRelatedQuests" or
...
function MM:GetOrCreate(key)
  ...
  if not pin then
    ...
    pin:SetScript("OnEnter",function()
      QuestieOcto.Tooltips:Show(this)
    end)
...
RefreshPinVisual=function(pin)
  ...
  for _,entry in pairs(pin.entries or {}) do
    if entry.node then ApplyVisual(pin,entry.node) end
  end
...
function MM:RemoveQuest(questID)
  ...
  self.stats.active=0
  for _,pin in pairs(self.frames) do
    if pin:IsShown() then self.stats.active=self.stats.active+1 end
  end
...
function MM:RefreshPlan()
  ...
  for _,desc in pairs(plan) do
    ...
    local pvpVisible=true
    if desc.type=="node" and desc.node and desc.node.pvp then
      pvpVisible=Settings():Get("showPvPRelatedQuests") and true or false
    elseif desc.type=="itemStartArea" and desc.area then
      local itemQuest=QuestieOcto.QuestModel:Get(desc.area.questID)
      if itemQuest and itemQuest.pvp then
        pvpVisible=Settings():Get("showPvPRelatedQuests") and true or false
      end
    end

    if role and IsRoleEnabled(role) and pvpVisible then
      local key=tostring(desc.key)
      local pin=self:GetOrCreate(key)
    ...
    if desc.type=="itemStartArea" and desc.area then
      pin.itemStartArea=desc.area
      pin.displayName=desc.area.displayName
      pin.clusterCount=desc.area.n or 1
      pin.mapX=tonumber(desc.area.x)
      pin.mapY=tonumber(desc.area.y)
      pin.role="itemStart"
      pin.questID=desc.area.questID
      local itemQuest=QuestieOcto.QuestModel:Get(desc.area.questID)
      pin.event=itemQuest and itemQuest.eventID and QuestieOcto.EventAvailability and QuestieOcto.EventAvailability:IsPresentationEvent(itemQuest.eventID) or false
      pin.pvp=itemQuest and itemQuest.pvp or false
      pin.repeatable=itemQuest and itemQuest.repeatable or false
      pin.iconScaleKey=nil
      ...
      if QuestieOcto.Visuals then
        QuestieOcto.Visuals:ApplyPin(pin,{role="itemStart",questID=desc.area.questID,pvp=pin.pvp,repeatable=pin.repeatable},true,pin.lastAlpha or 1)
      end
      ResizePin(pin)
      elseif desc.type=="node" and desc.node then
        pin.mapX=tonumber(desc.x)
        pin.mapY=tonumber(desc.y)
        pin.displayName=desc.node.sourceName or "Quest source"
        pin.clusterCount=math.max(pin.clusterCount or 1,desc.clusterCount or 1)
        AddEntry(pin,desc.node)
        if desc.kind=="objectiveFull" or desc.kind=="itemStartFull" then
          if QuestieOcto.Visuals and QuestieOcto.Visuals.ApplyFullNode then
            QuestieOcto.Visuals:ApplyFullNode(pin,desc.node,true,pin.lastAlpha or 1)
            pin.fullNodeNode=desc.node
          end
        end
      end
    end
  ...
  for key,pin in pairs(self.frames) do
    if not seen[key] then
      -- Match Questie's immediate note-frame removal semantics. Hiding alone
      -- is insufficient here because UpdatePositions() would re-show a stale
      -- frame that still retained its previous map coordinates.
      if pin:IsShown() then
        pin:Hide()
        self.stats.hidden=self.stats.hidden+1
      end
...
function MM:UpdatePositions(force)
  ...
  if self.planRevision~=QuestieOcto.PreparedMap.stateRevision then
    self:RefreshPlan()
    return
  end
  ...
  if not mapZoom or mapZoom<=0 then
    self:HideAll()
    return
  end
  ...
  for _,pin in pairs(self.frames) do
    ...
    if x and y then
      ...
      local xPos=dx*xDraw
      local yPos=dy*yDraw
      local pixelDistance=math.sqrt(xPos*xPos+yPos*yPos)

      -- Questie proper delegates minimap edge visibility to its pin library.
      -- pfQuest's Vanilla fallback waits until the whole icon fits inside the
      -- circle (distance + halfIcon < radius). For Questie-Octo's requested
      -- behavior, show every marker as soon as its WORLD COORDINATE enters the
      -- visible minimap circle. Projection stays unchanged.
      if pixelDistance<radius then
        pin.questieOctoBaseX=xPos
        pin.questieOctoBaseY=-yPos
        local groupKey=string.format("%.2f:%.2f",tonumber(x) or 0,tonumber(y) or 0)
  ...
  for _,group in pairs(visibleGroups) do
    ...
    for index,pin in ipairs(group) do
      ...
      pin:ClearAllPoints()
      pin:SetPoint("CENTER",Minimap,"CENTER",(pin.questieOctoBaseX or 0)+off[1],(pin.questieOctoBaseY or 0)+off[2])
    end
  end
